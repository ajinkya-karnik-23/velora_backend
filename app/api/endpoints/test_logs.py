...
async def list_test_logs(
    client_id: int | None = Query(
        default=None, description="Only entries for this client's cycles and controls."
    ),
    current_user: dict = Depends(get_current_user),  # noqa: ARG001
    db: AsyncSession = Depends(get_db),
) -> list[TestLogOut]:
    """Return the 200 most recent test log entries, optionally for one client."""
    service = TestLogService(db)
    return await service.list_all(client_id=client_id)
...
@router.post("/record-sample-run", response_model=TestLogOut, status_code=201)
async def record_sample_run(
    data: RecordSampleRunRequest,
    current_user: dict = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
) -> TestLogOut:
    """Record a sample run from the Testing tab in the audit log."""
    return await TestLogService(db).record_sample_run(data, current_user)
